...
import uuid
#example of a data entry we will be working with
# df_test = pd.DataFrame(data=[["2021-02-23 09:00:48","Isle of Wight","Morgan Berka","Large,Hot chocolate,2.9,Large,Chai latte,2.6,Large,Hot chocolate,2.9",
                               #   "CASH",8.40,"None"]]
...
def transform(data):
    ...
    list_of_dictionaries = data.to_dict('records') #Pandas is a fickle bitch and doesn't let us have lists inside cells so convert to dicitonaries like Gilly said
    for index,row in enumerate(list_of_dictionaries):
        list_of_dictionaries[index]["basket"] = format_orders(row)
...
def format_orders(row):
    try:
        basket_list = row.str.split(',').tolist()
        transactions = []
